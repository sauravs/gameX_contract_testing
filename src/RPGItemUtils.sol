...
contract RPGItemUtils {
    ...
    function stringEqual(string memory str1, string memory str2) internal pure returns (bool) {
        return bytes32(keccak256(abi.encode(str1))) == bytes32(keccak256(abi.encode(str2)));
    }

    function _generateStatHash(StatType memory _stat) internal pure returns (bytes32 hash) {
        assembly {
