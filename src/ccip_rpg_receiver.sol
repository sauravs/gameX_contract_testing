...
contract CCIP_RPG_RECEIVER is CCIPReceiver, OwnerIsCreator {
    ...
    function getHash(
        address contractAddress // used to generate the hash of given contract will used in verification that it is depolyed by us
    ) public view returns (string memory) {
        return uint256(keccak256(address(contractAddress).code)).toString();
    }

    function getMultipleHash(address[] calldata contractAddresses) public view returns (string[] memory) {
    ...
    function asciiToUint(
        string memory _asciiString //@audit RPG   // because in ccip we can only send strings this is for tha purpose // @audit can be made internal
    ...
    function addressToString(
        address _address //@audit RPG
    ) public pure returns (string memory) {
        return uint160(_address).toString();
    }

    function stringToAddress(
        string memory _string //@audit RPG
    ...
    function createMessageStr( // @audit @dev RPG    // here we craft stats in this chain (construct) and destruct it later on different chain
        ...
        require(stats.length == 4, "Stats array must contain exactly four elements");
        return string( // @audit @dev RPG
            abi.encodePacked(
                tokenId.toString(),
                ":",
    ...
    function readMessageStr(
        string memory _str // @audit @dev RPG deconstuct message on desitnation chain
    ) public pure returns (string[] memory messageParts) {
    ...
    function transferNft( //@audit RPG were you able to transfer nft?
        ...
        require(nftContract.isApprovedForAll(nftOwner, address(this)), "Token is not approved for transfer");
        ...
        (uint8 stat1, uint8 stat2, uint8 specialType, uint8 specialPoints) = nftContract.getTokenStats(_tokenId);
        ...
        string memory message = createMessageStr(
            _tokenId, nftOwner, senderNftContractAddress, destinationNftContractAddress, "TRANSFER", stats
        );
    ...
    function getFeeNative(uint64 destinationChainSelector, address receiver, string calldata _text)
        public
        view
        returns (uint256 fees)
    {
        Client.EVM2AnyMessage memory evm2AnyMessage = _buildCCIPMessage(receiver, _text, address(0), _gasLimit);
    ...
    function sendAcknowledgement(uint64 _destinationChainSelector, address _receiver, string memory _text)
        public
    ...
    function sendMessage(uint64 _destinationChainSelector, address _receiver, string calldata _text)
        ...
        returns (bytes32 messageId)
    {
        Client.EVM2AnyMessage memory evm2AnyMessage = _buildCCIPMessage(_receiver, _text, address(0), _gasLimit);
        ...
        onlyAllowlisted(any2EvmMessage.sourceChainSelector, abi.decode(any2EvmMessage.sender, (address)))
    {
        ...
        if (StringUtil.compareTo(messageParts[2], "TRANSFER")) {
            IRPGItemNFT rpgContract = IRPGItemNFT(stringToAddress(messageParts[4]));
            ...
            if (rpgContract.getOwner(tokenId) != address(0) && rpgContract.getOwner(tokenId) != newOwner) {
                rpgContract.transferFrom(rpgContract.getOwner(tokenId), newOwner, tokenId);
            }
            ...
            rpgContract.updateStats(tokenId, newOwner, stat1, stat2, specialType, specialPoints);
            ...
            this.sendAcknowledgement( //@audit
            any2EvmMessage.sourceChainSelector, abi.decode(any2EvmMessage.sender, (address)), message);
        } else if (StringUtil.compareTo(messageParts[2], "ACK")) {
            IRPGItemNFT rpgContract = IRPGItemNFT(stringToAddress(messageParts[3]));
